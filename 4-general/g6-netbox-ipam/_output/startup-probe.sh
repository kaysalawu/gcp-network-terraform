...
nping -c 5 --tcp-connect -p 80,8080 https://a-hub-eu-run-httpbin-wapotrwjpq-nw.a.run.app > /dev/null 2>&1
EOF
...
while [ $i -lt 5 ]; do
    ...
    ab -n $1 -c $2 https://a-hub-eu-run-httpbin-wapotrwjpq-nw.a.run.app > /dev/null 2>&1
    let i=i+1
  sleep 5
done
EOF
...
nping -c 5 -6 --tcp-connect -p 80,8080 https://a-hub-eu-run-httpbin-wapotrwjpq-nw.a.run.app > /dev/null 2>&1
EOF
