...
locals {
  ...
  netbox_startup_files = {
    ...
    "${local.netbox_app_dir}/start.sh"   = { owner = "root", permissions = "0744", content = templatefile("scripts/netbox/start.sh", local.netbox_vars) }
    "${local.netbox_app_dir}/stop.sh"    = { owner = "root", permissions = "0744", content = templatefile("scripts/netbox/stop.sh", local.netbox_vars) }
  }
}

module "netbox_cloud_init" {
  source = "../../modules/cloud-config-gen"
  files  = local.netbox_startup_files
  run_commands = [
