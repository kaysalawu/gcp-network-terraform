...
locals {
  egress_rules = {
    ...
    "${var.vpc_name}-4000-e-a-all-all-egress-internet" = {
      ...
      targets            = [var.target_tags.egress_internet, ]
    }
    ...
    "${var.vpc_name}-4200-e-a-all-all-all" = {
      ...
      rules              = [{ protocol = "all" }]
      targets            = [var.target_tags.egress_private, ]
    }
    # ipv6
    #*************************
  ...
  ingress_rules = {
    ...
    "${var.vpc_name}-4001-i-a-custom-internet-ingress" = {
      ...
      targets = [var.target_tags.ingress_internet, ]
    }
    "${var.vpc_name}-4201-i-a-all-private" = {
      ...
      rules         = [{ protocol = "all" }]
      targets       = [var.target_tags.ingress_private, ]
    }
