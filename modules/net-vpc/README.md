...
| <a name="input_factories_config"></a> [factories\_config](#input\_factories\_config) | Paths to data files and folders that enable factory functionality. | <pre>object({<br>    context = optional(object({<br>      regions = optional(map(string), {})<br>    }), {})<br>    subnets_folder = optional(string)<br>  })</pre> | `{}` | no |
...
| <a name="input_network_attachments"></a> [network\_attachments](#input\_network\_attachments) | PSC network attachments, names as keys. | <pre>map(object({<br>    subnet                = string<br>    automatic_connection  = optional(bool, false)<br>    description           = optional(string, "Terraform-managed.")<br>    producer_accept_lists = optional(list(string))<br>    producer_reject_lists = optional(list(string))<br>  }))</pre> | `{}` | no |
...
| <a name="output_network_attachment_ids"></a> [network\_attachment\_ids](#output\_network\_attachment\_ids) | IDs of network attachments. |
...
| <a name="output_subnets_private_nat"></a> [subnets\_private\_nat](#output\_subnets\_private\_nat) | Private NAT subnet resources. |
